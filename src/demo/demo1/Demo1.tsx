...
function Demo1RouteNav({ routes }: Demo1Props) {
  return (
    <nav className="demo1-route-nav" aria-label="Demo variants">
      {routes.map((route) => (
        <a aria-current={route.id === "demo1" ? "page" : undefined} data-testid={`demo-route-${route.id}`} href={route.path} key={route.id}>
          {route.label}
        </a>
      ))}
    </nav>
...
export function Demo1({ routes }: Demo1Props) {
  ...
  const [snapMinutes, setSnapMinutes] = useState(10);
  ...
  const [message, setMessage] = useState("Compact horizontal board");
  ...
  const handleScaleChange = (nextScale: number) => {
    ...
    setMessage(`Loaded ${nextScale.toLocaleString()} compact events/year`);
  };
  ...
  const handleCreate = useCallback((request: EventCreateRequest) => {
    ...
    setMessage("Created compact appointment");
  }, []);

  return (
    <main className="demo1-shell" data-demo-id="demo1">
      <aside className="demo1-sidebar" aria-label="Demo controls">
        <div className="demo1-brand">
          <CalendarDays size={24} aria-hidden />
          <div>
            <strong>Compact Board</strong>
            <span>Horizontal clinic throughput</span>
          </div>
        </div>

        <Demo1RouteNav routes={routes} />
        ...
        <fieldset className="demo1-view-switch" aria-label="Calendar type">
        ...
        <label>
          Zoom
          <div className="demo1-zoom-control">
        ...
        <div className="demo1-time-range">
          <label>
            Start
        ...
        <p className="demo1-message" data-testid="demo-message">
          {message}
        </p>
      </aside>

      <section className="demo1-calendar-panel">
        <CalendarRoot
          key={scale}
          view={calendarView}
          ref={calendarRef}
