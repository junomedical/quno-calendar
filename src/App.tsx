...
function countVisibleEventNodes() {
  ...
  return Array.from(document.querySelectorAll<HTMLElement>('[data-testid="calendar-event"], [data-testid="availability-event"], [data-testid="draft-event"]')).filter(
    (element) => {
      const box = element.getBoundingClientRect();
      return box.width > 0 && box.height > 0 && box.right > viewport.left && box.left < viewport.right && box.bottom > viewport.top && box.top < viewport.bottom;
    }
  ).length;
}
