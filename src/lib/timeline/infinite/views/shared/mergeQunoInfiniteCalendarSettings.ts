...
import { MAX_ZOOM } from "#quno-internal/timeline/infinite/interactions/zoom/zoomLimits";

/** Merges caller settings with defaults and clamps invalid timeline values. */
export function mergeQunoInfiniteCalendarSettings(
  settings?: Partial<QunoInfiniteCalendarSettings>
): QunoInfiniteCalendarSettings {
  const merged = { ...defaultQunoInfiniteCalendarSettings, ...settings };
  return {
    ...merged,
    endHour: Math.max(merged.startHour + 1, merged.endHour),
    snapMinutes: Math.max(1, merged.snapMinutes),
    zoom: Number.isFinite(merged.zoom) ? Math.min(MAX_ZOOM, merged.zoom) : defaultQunoInfiniteCalendarSettings.zoom,
