...
type WheelZoomCommit = (args: { zoom: number }) => void;

/** Accumulates raw wheel/touchpad steps and runs only the latest complete projection before a paint. */
export function useFrameCoalescedWheelZoom({ controlledZoom }: { controlledZoom: number }) {
    ...
    ({
      ...
      if (!frameRef.current) {
        frameRef.current = window.requestAnimationFrame(() => {
          ...
          if (pendingZoom !== null && controlledZoomRef.current === pendingBaseZoom)
            pendingCommit?.({ zoom: pendingZoom });
        });
      }
      return nextZoom;
    },
...
export function restoreIsCurrent({ versionRef, version }: { versionRef: MutableRefObject<number>; version: number }) {
  return versionRef.current === version;
}

/** Reapplies a scroll anchor while controlled React layout settles. */
export function restoreAcrossFrames({ restore, frameCount }: { restore: () => void; frameCount: number }) {
  let cancelled = false;
  let frame = 0;
  const run = () => {
    if (cancelled) return;
 ...
 */
export function scheduleZoomCommit({
  commit,
  restore,
  restoreFrameCount
}: {
  ...
  queueMicrotask(() => {
    commit();
    window.requestAnimationFrame(() => restoreAcrossFrames({ restore, frameCount: restoreFrameCount }));
  });
}

export function useCapturedWheel({
  ref,
  listener
}: {
