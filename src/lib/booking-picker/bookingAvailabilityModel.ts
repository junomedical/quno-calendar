...
export type QunoBookingQueryPeriod = { start: string; end: string };
type BookingEnvelope<TSlot extends QunoComparableBookingSlot, TPeriod extends QunoBookingQueryPeriod> = {
  booking_availability: { slots: TSlot[]; query_periods: TPeriod[] };
};
...
const instantMinute = (value: string): string => {
  const instant = Date.parse(value);
  return Number.isFinite(instant) ? String(Math.trunc(instant / 60_000)) : value;
};
const intervalMinuteKey = ({ start, end }: QunoComparableBookingSlot): string =>
  `${instantMinute(start)}|${instantMinute(end)}`;
...
>(
  cronofy: TEnvelope,
  quno: TEnvelope
): { options: TEnvelope; counts: QunoAvailabilityComparisonCounts } => {
    ...
    options: {
      ...
      booking_availability: {
        ...
        query_periods: mergeBookingQueryPeriods(
          cronofy.booking_availability.query_periods,
          quno.booking_availability.query_periods
        ),
        slots
      }
    },
