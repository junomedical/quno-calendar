...
sequenceDiagram
  ...
  Container-->>Virtual: scrollend or idle debounce
...
`useEventRangeLoader` owns this async boundary. It computes the smallest missing visible range, tracks loaded/loading date keys, cancels stale generations when the loader, selected calendars, or optional `eventVersion` change, and stores results by event start date. Range loads can include dates that are already present between missing dates, so each date bucket is upserted by event id instead of blindly appended. Rendering components read only `eventsByDate`.
...
Overlapping events are laid out with sorted interval partitioning. A connected overlap chain shares one mini-height lane count, and each event uses the first lane whose previous event has ended. Compact rows default to 50px, and two overlapping events still keep that compact height. Overlap depth grows only the affected date/calendar row from three lanes onward with the stepped ladder, then clamps the result so every dense overlap lane has at least 24px. Resting shells are inset 2px from the top and bottom of their mini-lane, so dense event shells stay at least 20px tall and adjacent resting lanes do not touch. On hover, the shell expands to the full calendar row lane height and comes forward above neighboring shells.

Variable day heights use a cheap base estimate in the virtualizer and measured rendered day elements for actual size. This avoids scanning the full virtual date range while still letting individual dense days become taller. New-event drafts render as overlay shells in the target row and are excluded from overlap lane assignment, row-height metrics, and committed row event counts until creation succeeds. Active edit drafts also stay out of metrics: the loaded source event is filtered before row-height calculation, then the controlled draft is rendered as an overlay replacement so opening or moving the popup draft cannot add another overlap lane.
...
The vertical view computes one grid column per selected calendar for each rendered date. Columns fill available width when there is room and start from `settings.verticalColumnMinWidth`. Timed event overlaps use horizontal lanes inside the column. `settings.verticalColumnOverlapCapacity` controls how many parallel lanes fit inside the base width; every additional lane adds `settings.verticalColumnOverlapGrowth` to that date/calendar column. Active edit draft source events are filtered before this width calculation, so the draft overlay does not widen a column while the popup is open. The sticky doctor-name header for the same date uses the same grid template as the body columns, so a locally widened column also widens its title cell. Hovered vertical events use `settings.verticalEventHoverMinHeight` as their readable minimum height.
...
Popup code decides whether a draft edit should focus the event or preserve the current screen position. The default demo records the clicked event's viewport-relative position when edit opens, then keeps updating that snapshot from visible draft instances. Same-date time edits do not scroll when the draft is already visible. Date changes first render the draft in its natural destination date; if that destination is visible, scroll is left alone, and if it is offscreen the demo restores the preview to the last seen viewport-relative position. Participant filtering, drawn create handoff, save, and cancel also snapshot the draft and restore the replacement, saved, or original event to the relevant viewport-relative position. Visible row-only participant changes and visible cancel restores disable date/time centering fallback so the draft or restored original is not parked in the viewport center. Pending delayed restore corrections are cancelled by manual viewport scrolls and by newer draft geometry edits, so stale offscreen refocus work cannot move a draft after it has returned to a visible date. During drawn-create delegation, the library keeps the local draft mounted until the next animation frame so the parent-controlled `activeDraft` can take over without a blank or shifted intermediate paint. Demo saves simulate a delayed server boundary in the parent: the popup disables while pending, successful saves commit after the delay, bump `eventVersion` so the loaded visible-range cache refreshes from the parent event store, and titles containing `fail` reopen/keep the draft editable with a small bottom error message.
