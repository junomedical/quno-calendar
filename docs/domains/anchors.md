...
flowchart TD
  ...
  Priority -->|Controlled zoom| Zoom["Visible center time"]
...
| [`useHorizontalControlledZoomAnchor.ts`](../../src/lib/infinite/anchors/zoom/useHorizontalControlledZoomAnchor.ts) | Preserves horizontal center time across controlled zoom prop changes.         |

## Verification Map

- Unit: geometry registry, data-layout anchor, controlled zoom anchor, and resize compensation.
- Browser: async height growth, external layout restore/cancel, date/time navigation, zoom continuity, and interaction priority.
