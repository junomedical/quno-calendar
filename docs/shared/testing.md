...
- `className`, `style`, `ariaLabel`, and `initialDateKey` work through `QunoInfiniteCalendar` in both orientations.
- The architecture guard rejects every parent-directory module import across production, demo, unit, and browser-test
  files, requiring stable public or `#quno-*` responsibility aliases for cross-folder dependencies.
...
- Consumer-defined semantic color variables compute on the shell, headers, labels, grids, time text, borders, shadow,
  and current-time marker in both orientations when inherited from outside `QunoInfiniteCalendar`.
...
- The architecture check keeps `src/` library-only and requires the integration field guide to provide a README,
  source backlink, and `@quno/calendar/infinite-calendar` public-package import.
...
- The React-state chapter switches controlled calendar visibility and zoom through product-owned state. Its summary,
...
- Demo routes `/demo/infinite-calendar`, `/demo1`, `/demo2`, and `/demo3` render distinct calendar compositions using
  ...
  `/demo/datepicker`, `/demo/date-input`, and `/demo/date-parser` routes render only their named primitives. The parser
  playground exposes phrases, options, tokens, and resolved output. The primary infinite
