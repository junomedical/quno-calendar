...
| Product name           | Public surface                     | Responsibility                                                   |
| ---------------------- | ---------------------------------- | ---------------------------------------------------------------- |
...
| ---------------------------- | ---------------------------------------------------------------------------------------------------- | --------------------------------------------------------- |
| Calendar surface             | The whole reusable calendar component area, excluding demo sidebar controls.                         | public `QunoInfiniteCalendar`; internal orientation views |
| View                         | A swappable calendar presentation inside `QunoInfiniteCalendar`.                                     | `view`                                                    |
...
| Calendar theme               | Consumer-owned semantic colors inherited by calendar chrome in both orientations.                    | `--quno-calendar-*`, `QunoInfiniteCalendarStyle`          |
...
| Parent viewport anchor    | An explicit event or slot geometry snapshot captured/restored through `QunoInfiniteCalendarHandle` for product-owned UI changes.                                       | Parent flow plus anchor restore runtime  |
| Event focus request       | A one-shot request that reveals known participants, positions one preferred event instance, and briefly highlights it without taking DOM focus.                        | `QunoInfiniteCalendar` focus coordinator |
