...
These guides expand the [Infinite Calendar architecture](../architecture.md) into concrete runtime stories. Each guide
follows a trigger through state ownership, async boundaries, layout work, viewport correction, cancellation, and
rendering. The diagrams describe library behavior; examples and product UI remain consumers of that behavior.

For the complementary ownership view and a source-file-by-source-file map, see the
