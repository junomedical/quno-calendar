...
describe("DST mutation safety", () => {
  it.each([
    ...
    expect(() => zonedDateMinuteToIso(date, minute, zone)).toThrow("ambiguous");
  });
  it.each([
    ...
    expect(() => zonedDateMinuteToIso(date, minute, zone)).toThrow("does not exist");
  });
  it.each([
    ...
    const proposal = buildMoveProposal(
      { id: "event", title: "Appointment", calendarId: "doctor", start, end, calendarTimeZone: "Europe/Berlin" },
      { dateKey, calendarId: "doctor", minute, dayIndex: 0, rowIndex: 0 },
      0,
      { startHour: 0, endHour: 24, snapMinutes: 5, timeZone: "Europe/Berlin" }
    );
